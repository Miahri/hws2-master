...
function Clock() {
    ...
    const start = () => {
        ...
        let tId = setInterval(function timer() {
            setDate(new Date(restoreState('hw9-date', Date.now())));
            setTimerId(Number(tId));
        }, 1000);
    ...
    const stringDate = addZero(date.getDate()) + '.' + addZero(date.getUTCMonth() + 1) + '.' + addZero(date.getFullYear()) || <br/> // день.месяц.год (01.02.2022) // пишут студенты, варианты 01.02.0123/01.02.-123/01.02.12345 не рассматриваем
